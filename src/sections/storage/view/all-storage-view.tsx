...
export function AllStorageView() {
  ...
  const handleNotifyDestroy = () => {
    setNotification('The selected storage is being destroyed.');
  ...
  useEffect(() => {
    const fetchStorages = async () => {
      try {
        ...
        setConnectionStatus({ minio: status });
        
        if (!status) {
          console.warn('MinIO backend servisine bağlanılamıyor. Backend çalışmıyor olabilir.');
          setStorages([]); // Empty list instead of error
          return;
        }
        ...
        console.warn('MinIO storage fetch error:', err);
        setStorages([]); // Show empty list instead of error
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleRefresh = () => {
    const fetchStorages = async () => {
      try {
        ...
        console.warn('MinIO storage refresh error:', err);
        setStorages([]);
      } finally {
        setLoading(false);
      }
  ...
  const canCreate = connectionStatus.minio;
